...
export const batch1: ContentPost[] = [
  ...
  },
  {
    ...
    primaryLink: 'https://platform.claude.com/docs/en/agent-sdk/overview',
    primaryLabel: 'Doc oficial do Agent SDK',
    isExternal: true,
    author: '@joaoguirunas',
    sourceUrl: 'https://platform.claude.com/docs/en/agent-sdk/overview',
    longDescription: [
      'O lançamento que mais mexeu com quem constrói com IA foi o Claude Agent SDK, da Anthropic: o mesmo motor que roda por trás do Claude Code, agora disponível como biblioteca para você criar agentes próprios em TypeScript ou Python. Em vez de só conversar com o modelo, você programa um agente que lê arquivos, roda comandos e edita código por conta própria.',
      'O ponto que muda o jogo é o "agent loop" pronto: o SDK já vem com ferramentas embutidas (ler arquivos, executar comandos no terminal, editar código), então o agente começa a trabalhar sem você ter que implementar a execução de cada ferramenta na mão. É a infraestrutura de um agente de produção entregue de fábrica.',
      'Este é um recap de notícia, não um tutorial de instalação passo a passo: o objetivo é você entender o que foi anunciado, por que importa e por onde começar. Para o how-to completo, a doc oficial linkada abaixo tem o quickstart.',
    ],
    features: [
      {
        title: 'Mesmo motor do Claude Code',
        description: 'O SDK expõe a engine que já roda o Claude Code — agentes com a mesma capacidade, agora programáveis por você.',
        icon: 'framework',
      },
      {
        title: 'TypeScript e Python',
        description: 'Instale com npm install @anthropic-ai/claude-agent-sdk ou pip install claude-agent-sdk (Python 3.10+).',
        icon: 'agents',
      },
      {
        title: 'Ferramentas embutidas',
        description: 'Read, Edit e Bash já vêm prontos — o agente lê arquivos, edita código e roda comandos sem você implementar a execução.',
        icon: 'setup',
      },
      {
        title: 'Agent loop pronto',
        description: 'O laço de raciocínio-ação-observação vem implementado; você descreve a tarefa e o agente itera sozinho até concluir.',
        icon: 'automation',
      },
      {
        title: 'Sessões persistentes',
        description: 'Suporte a sessões que podem ser salvas em armazenamento externo, para agentes que retomam contexto entre execuções.',
        icon: 'database',
      },
      {
        title: 'Multi-provider',
        description: 'Autentica via API da Anthropic ou por provedores como Amazon Bedrock e Google Vertex, conforme sua infra.',
        icon: 'deploy',
      },
    ],
    body: [
      { type: 'paragraph', text: 'Por que este foi "o maior lançamento da semana"? Porque a Anthropic abriu como biblioteca a mesma tecnologia que faz o Claude Code funcionar. Quem queria construir um agente de IA de verdade — que age, não só responde — antes tinha que montar toda a orquestração na mão. O Agent SDK entrega isso pronto.' },
      { type: 'heading', text: 'O que é o Claude Agent SDK' },
      { type: 'paragraph', text: 'É o SDK oficial da Anthropic para construir agentes autônomos com Claude, disponível em TypeScript e Python. Ele inclui ferramentas embutidas para ler arquivos, rodar comandos e editar código, então seu agente já começa a trabalhar sem você implementar a execução de cada ferramenta.' },
      { type: 'heading', text: 'O destaque: agente que age sozinho' },
      { type: 'paragraph', text: 'O exemplo da própria doc resume bem o salto — você dá um objetivo e libera as ferramentas, e o agente executa o ciclo inteiro:' },
      { type: 'code', language: 'python', code: 'async for message in query(\n    prompt="Find and fix the bug in auth.py",\n    options=ClaudeAgentOptions(allowed_tools=["Read", "Edit", "Bash"]),\n):\n    print(message)\n# Claude lê o arquivo, encontra o bug e corrige' },
      { type: 'heading', text: 'Por onde começar' },
      { type: 'steps', items: [
        'Instale o SDK: npm install @anthropic-ai/claude-agent-sdk (ou pip install claude-agent-sdk, Python 3.10+).',
        'Configure a chave: export ANTHROPIC_API_KEY=sua-chave (ou use Bedrock/Vertex).',
        'Siga o quickstart da doc oficial para montar um agente que corrige bugs em minutos.',
      ] },
      { type: 'callout', text: 'Recap de notícia: este card resume o anúncio e aponta a fonte oficial. O passo a passo completo (quickstart, exemplos de agentes, sessões) está na documentação do Agent SDK linkada acima.' },
    ],
  },
  {
