...
export interface CursoEmBreveFaqProps {
  items: FaqItem[]
  id?: string
}

export function CursoEmBreveFaq({ items, id = 'faq' }: CursoEmBreveFaqProps) {
              ...
              >
                <h3
                  className={`text-sm sm:text-base font-semibold transition-colors leading-tight ${
                    openIndex === i ? 'text-[#FF3A0E]' : 'text-white'
                  }`}
                >
                  {item.q}
                </h3>
                <svg
                  className={`h-4 w-4 text-[#FF3A0E] transition-transform flex-shrink-0 ${
